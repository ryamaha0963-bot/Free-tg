    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS orders (
                ...
                diamonds_spent INTEGER DEFAULT 10,
        ...
        await db.execute("CREATE INDEX IF NOT EXISTS idx_accounts_sold ON accounts(is_sold)")
        
        # Add columns if missing (backward compatibility)
        try:
...
async def add_referral(referrer_id: int, referred_id: int) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Add 1 diamond to referrer's wallet
        await db.execute(
...
async def get_earned_accounts(user_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute("""
            SELECT a.*, o.id as order_id, o.created_at as claimed_at, o.diamonds_spent
            FROM orders o
...
async def claim_account_for_user(user_id: int) -> dict | None:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute(
            "INSERT INTO orders (user_id, account_id, diamonds_spent, status) VALUES (?, ?, 10, 'claimed')",
            (user_id, account['id'])
        )
        await db.commit()
