    ...
    def __init__(self, app: Client):
                            ...
                            if count % 2 == 0:
                                ...
                                if not available:
                                    ...
                                    if account:
                                        ...
                                        # Claim failed (shouldn't happen if available check passed)
                                        await client.send_message(
                                            referrer['user_id'],
                                            "❌ **Account claim failed due to a database error.**\n"
                                        ...
                                        try:
                                            await client.send_message(
                                                Config.ADMIN_ID,
                                                f"❌ Claim failed for user {referrer['user_id']} with {count} referrals. Check DB."
                                            )
                                        except:
                                            pass
                            else:
        ...
        async def callback_handler(client, callback: CallbackQuery):
            ...
            if data == "stats":
                ...
                if remaining == 0:
                    remaining = 2
                # Progress bar: shows progress towards next account (0/2)
