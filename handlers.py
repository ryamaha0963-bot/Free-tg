...
import logging, asyncio, aiosqlite, traceback, re
...
# ========== IMAGE MAPPING (Welcome & Profile swapped) ==========
MEDIA_MAPPING = {
    ...
    def __init__(self, app: Client):
                ...
                if msg.entities:
                    for entity in msg.entities:
                        if hasattr(entity, 'custom_emoji_id') and entity.custom_emoji_id:
                            ...
                            return
                # Check caption entities (if photo/video)
                if msg.caption_entities:
                    for entity in msg.caption_entities:
                        if hasattr(entity, 'custom_emoji_id') and entity.custom_emoji_id:
                            ...
                            return
                await message.reply("❌ No custom emoji found in the replied message.", parse_mode=ParseMode.MARKDOWN)
            else:
                await message.reply(
                    "📌✨ **Usage:** Reply to a message containing a Premium emoji with `/getemojioid`",
                    parse_mode=ParseMode.MARKDOWN
                )
        ...
        async def admin_cmd(client, message):
            await message.reply(
                ...
                "• `/getfileid` – 🖼️ Get file ID (reply to photo)\n"
                "• `/getemojioid` – 🎨 Get premium emoji ID (reply to message)",
                parse_mode=ParseMode.MARKDOWN
            )
...
# ---------- OTP FORWARDER (PERMANENT, USING TELETHON) ----------
