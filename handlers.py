...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def start_cmd(client, message):
            ...
            if len(message.command) > 1:
                ...
                logger.info(f"Referral param: {ref_param}")
                if ref_param.startswith("ref_"):
                    ref_code = ref_param[4:]  # strip 'ref_'
                    referrer = await get_user_by_referral_code(ref_code)
                    if referrer:
                        logger.info(f"Referrer found: {referrer['user_id']}")
                        if referrer['user_id'] != user_id:
                            ...
                            if success:
                                ...
                                try:
                                    await client.send_message(
                                        ...
                                        f"🎯 Next account in: **{2 - (count % 2)}** more.",
                                ...
                                if count % 2 == 0:
                                    ...
                                    if not available:
                                        await client.send_message(
                                            ...
                                            "Admin will add more soon. Your referrals are saved, you'll get your account when stock is updated.",
                                            parse_mode=ParseMode.MARKDOWN
                                        )
                                    else:
                                        account = await claim_account_for_user(referrer['user_id'])
                                        if account:
                                            phone = account.get('phone') or 'N/A'
                                            password = account.get('password') or 'N/A'
                                            otp = account.get('otp') or 'N/A'
                                            session_str = account.get('session_string') or 'N/A'
                                            creds = (
                                                f"🎁 **Congratulations!** You've earned a new account!\n\n"
                                                f"📱 **Phone:** `{phone}`\n"
                                                f"🔑 **Password:** `{password}`\n"
                                                f"🔐 **OTP Backup:** `{otp}`\n"
                                            ...
                                            await client.send_message(
                                                referrer['user_id'],
                                                "❌ **Account claim failed.** Please contact admin.",
                                                parse_mode=ParseMode.MARKDOWN
                                            )
                                else:
                                    remaining = 2 - (count % 2)
                            ...
                            # User used their own link
                            await message.reply("😄 You can't refer yourself!", parse_mode=ParseMode.MARKDOWN)
                    else:
                        # Invalid referral code
                        await message.reply(
                            "❌ **Invalid referral link.**\n"
                            "Please use the link provided by the bot.",
                            parse_mode=ParseMode.MARKDOWN
                        )
                else:
                    # Parameter present but not a referral
                    pass
        ...
        async def available_cmd(client, message):
            ...
            await message.reply(
                ...
                f"Total accounts in pool: {count}",
                parse_mode=ParseMode.MARKDOWN
            )
        ...
        async def gen_session_cmd(client, message):
            ...
            if message.from_user.id in self.pending_sessions:
                await message.reply(
                    "⏳ Already generating a session. Complete it first.",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
        ...
        async def complete_otp_cmd(client, message):
            ...
            if not session_data:
                await message.reply(
                    "❌ No pending session. Use `/gensession` first.",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
            if session_data["phone"] != phone:
                await message.reply(
                    f"❌ Phone mismatch. Expected `{session_data['phone']}`",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
            ...
            try:
                ...
                await message.reply(
                    ...
                    f"📱 Phone: `{phone}`\n"
                    f"🔑 Session String:\n`{session_string}`\n\n"
                    f"Now use `/addaccount` with this session.",
                    parse_mode=ParseMode.MARKDOWN
                )
            except Exception as e:
                await message.reply(f"❌ Failed: `{e}`", parse_mode=ParseMode.MARKDOWN)
        ...
        async def add_account_cmd(client, message):
            ...
            if len(parts) < 6:
                await message.reply(
                    ...
                    "Example:\n"
                    "`/addaccount +911234567890 MyPass123 456789 session_here \"Premium Account\"`",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
            ...
            await message.reply(
                f"✅ **Account #{acc_id} added successfully!**",
                parse_mode=ParseMode.MARKDOWN
            )
        ...
        async def update_otp_cmd(client, message):
            ...
            if len(parts) != 3:
                await message.reply(
                    "📌 **Usage:** `/updateotp <account_id> <new_otp>`\nExample: `/updateotp 1 987654`",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
            ...
            await message.reply(
                f"✅ OTP for account #{acc_id} updated to `{new_otp}`",
                parse_mode=ParseMode.MARKDOWN
            )
        ...
        async def ref_stats_cmd(client, message):
            ...
            if len(parts) != 2:
                await message.reply(
                    "📌 **Usage:** `/refstats <user_id>`\nExample: `/refstats 123456789`",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
            uid = int(parts[1])
    ...
    async def _send_force_join_message(self, client, message):
        text = (
            ...
            "To use this bot, you must join our **Channel** and **Group** below:\n\n"
            "After joining, click the **I have joined** button to continue."
        )
        buttons = []
        if Config.FORCE_CHANNEL:
...
async def forward_telegram_otp(account_id: int, buyer_id: int, session_string: str):
    if not session_string or len(session_string) < 10:
        logger.warning(f"Session string missing for account {account_id}")
        return
