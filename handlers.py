...
# ==================== IMAGE FILE IDs ====================
# 👇 Saare IDs alag‑alag set hain – koi mix‑up nahi
MEDIA_MAPPING = {
...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def start_cmd(client, message):
            ...
            # Show Welcome screen
            await self._show_screen(client, message, "welcome", user_id)
        ...
        async def callback_handler(client, callback):
            ...
            if data == "auto_switch":
                ...
                await self._show_screen(client, old_msg, "welcome", user_id)
                return
            ...
            # ---------- SCREEN NAVIGATION (Explicit Mapping) ----------
            # Har callback data ka ek specific screen name hai
            if data == "welcome":
                await self._show_screen(client, old_msg, "welcome", user_id)
            elif data == "profile":
                await self._show_screen(client, old_msg, "profile", user_id)
            elif data == "invite":
                await self._show_screen(client, old_msg, "invite", user_id)
            elif data == "rewards":
                await self._show_screen(client, old_msg, "rewards", user_id)
            elif data == "my_rewards":
                await self._show_screen(client, old_msg, "my_rewards", user_id)
            elif data == "progress":
                await self._show_screen(client, old_msg, "progress", user_id)
            elif data == "support":
                await self._show_screen(client, old_msg, "support", user_id)
            elif data == "admin":
                await self._show_screen(client, old_msg, "admin", user_id)
            elif data == "admin_broadcast":
                await self._show_screen(client, old_msg, "admin_broadcast", user_id)
            else:
    ...
    async def _show_screen(self, client, target, screen, user_id):
        """Delete old message and send new one with exact image + caption for the given screen."""
        # Delete old message
        try:
            if hasattr(target, 'delete') and target:
                await target.delete()
        except:
            pass
        ...
        try:
            ...
            logger.error(f"Photo failed for {screen}: {e}")
            await client.send_message(
                chat_id=chat_id,
