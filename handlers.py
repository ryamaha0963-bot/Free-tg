...
MEDIA_MAPPING = {
    ...
    "my_rewards": "AgACAgUAAxxkBAAMVzWf_4_REWARDS_ASCEND",  # same as rewards
...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def start_cmd(client, message):
                    ...
                    if referrer and referrer['user_id'] != user_id:
                        ...
                        if success:
                            ...
                            await client.send_message(
                                ...
                                f"💎 +1 diamond! Total: **{diamonds}**",
                                parse_mode=ParseMode.MARKDOWN
                            )
                        else:
                            await client.send_message(
                                referrer['user_id'],
                                "❌ Already referred by someone else.",
                                parse_mode=ParseMode.MARKDOWN
                            )
                    else:
                        if referrer and referrer['user_id'] == user_id:
            ...
            # Show welcome screen with image
        ...
        async def _show_screen(self, client, target, screen, user_id, is_edit=True):
            ...
            if screen == "welcome":
                ...
                # Update caption with actual link
                ...
                keyboard = [
                    support_btn,
                    [InlineKeyboardButton("🔙 Back to Home", callback_data="welcome")]
                ] if support_btn else [[InlineKeyboardButton("🔙 Back to Home", callback_data="welcome")]]
        ...
        async def callback_handler(client, callback: CallbackQuery):
            ...
            if data == "auto_switch":
                ...
                state = "ON" if AUTO_SWITCH_STATE[user_id] else "OFF"
                await callback.answer(f"Auto-Switch {state}")
                # Go back to welcome to show updated button
            ...
            # ---- ADMIN USERS / STATS / SETTINGS (placeholder) ----
        ...
        async def add_account_cmd(client, message):
            ...
            if len(parts) < 6:
                await message.reply(
                    "Usage: /addaccount <phone> <pass> <otp> <session> <desc>",
                    parse_mode=ParseMode.MARKDOWN
                )
                return
...
async def forward_telegram_otp_telethon(account_id: int, buyer_id: int, session_string: str):
    ...
    try:
        ...
        await _bot_instance.send_message(
            buyer_id,
            f"❌ OTP listener crashed. Click 'Get OTP' to restart.",
            parse_mode=ParseMode.MARKDOWN
        )
    finally:
        try:
