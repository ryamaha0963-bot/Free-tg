...
import logging, asyncio, aiosqlite, traceback, io, re
...
# ========== HELPER: Convert text to premium emoji entities ==========
def text_to_premium_entities(text):
    """
    Scans text for standard emojis that have premium IDs, returns (new_text, entities).
    """
    entities = []
    new_text = text
    offset_shift = 0
    # We'll iterate over the string and find emojis
    # Using regex for emoji detection: simple pattern for our mapped emojis
    pattern = '|'.join(re.escape(emoji) for emoji in PREMIUM_EMOJI_MAP.keys())
    matches = re.finditer(pattern, text)
    for match in matches:
        emoji = match.group(0)
        emoji_id = PREMIUM_EMOJI_MAP[emoji]
        start = match.start() + offset_shift
        length = len(emoji)
        # Replace emoji with a placeholder (we will keep the emoji character for visual, but we add entity)
        # Actually, we keep the emoji character as is, and add custom entity.
        # Pyrogram will render it as custom emoji if we provide entity.
        # So we don't replace the text.
...
MEDIA_MAPPING = {
    ...
    "profile": "AgACAgUAAxkBAAICQGpiIin66ZiZAAE8w5w-TfLrYKGNFwACGhNrG6YuEFcAARIpUDBtUDkACAEAAwIAA3kABx4E",
...
# ========== OVERLAY FUNCTION ==========
async def overlay_profile_info(client, user_id, base_file_id):
    try:
        img_data = await client.download_media(base_file_id, in_memory=True)
        if not img_data:
            return None
        img = Image.open(img_data).convert("RGBA")
        overlay = Image.new('RGBA', img.size, (0, 0, 0, 0))
        draw = ImageDraw.Draw(overlay)
        box_width = img.width - 80
        box_height = 280
        box_x = 40
        box_y = img.height - box_height - 40
        draw.rectangle(
        ...
        next_target = DIAMOND_COST - (diamonds % DIAMOND_COST)
        if next_target == DIAMOND_COST:
            next_target = 0
        ...
        y = box_y + 15
        draw.text((box_x + 20, y), f"👤 {username}", font=font_title, fill=(255, 255, 255))
        y += 45
        draw.text((box_x + 20, y), f"💎 Diamonds: {diamonds}", font=font_medium, fill=(255, 215, 0))
        y += 35
        draw.text((box_x + 20, y), f"👥 Referrals: {referrals}", font=font_medium, fill=(200, 200, 255))
        y += 35
        draw.text((box_x + 20, y), f"📱 Accounts Earned: {earned_count}", font=font_medium, fill=(200, 255, 200))
        y += 35
        draw.text((box_x + 20, y), f"🎯 Next Target: {next_target} diamonds", font=font_medium, fill=(255, 200, 200))
...
class BotHandlers:
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=False):
        ...
        # ===== CAPTIONS (with standard emojis – will be replaced automatically) =====
        raw_captions = {
            "welcome": (
                "🌟💎 **Welcome to ASCEND** 💎🌟\n\n"
        ...
        if screen == "welcome":
            ...
            raw_captions["invite"] = raw_captions["invite"].format(bot_username, user['referral_code'])
            keyboard = [
        ...
        caption_raw = raw_captions.get(screen, "")
        image_id = MEDIA_MAPPING.get(screen)

        chat_id = target.chat.id if hasattr(target, 'chat') else target.chat.id

        # ===== PROFILE SCREEN: OVERLAY IMAGE =====
        if screen == "profile" and image_id:
            try:
                img_bytes = await overlay_profile_info(client, user_id, image_id)
                if img_bytes:
                    # For overlay image, we send simple caption with premium emoji
                    cap_text, cap_entities = text_to_premium_entities("👤✨ Your Dynamic Profile")
            ...
            # Fallback: send text with info
            fallback_text = f"👤✨ **Your Profile**\n\n💎 Diamonds: {diamonds}\n👥 Referrals: {count}\n📱 Accounts Earned: {len(earned)}\n🎯 Next Target: {DIAMOND_COST - (diamonds % DIAMOND_COST)} diamonds"
            # Convert to premium entities
            fb_text, fb_entities = text_to_premium_entities(fallback_text)
        ...
        # ===== OTHER SCREENS: STATIC IMAGE WITH PREMIUM CAPTION =====
        cap_text, cap_entities = text_to_premium_entities(caption_raw)
        try:
            if image_id:
                await client.send_photo(
                    chat_id=chat_id,
            ...
            # Fallback: send without custom entities (plain text)
            await client.send_message(
                chat_id=chat_id,
    ...
    async def _send_force_join_message(self, client, message):
        text_raw = "🔐✨ **Verification Required** ✨🔐\n\nJoin our Channel & Group below:\n\nAfter joining, click **'I have joined'**."
        text, entities = text_to_premium_entities(text_raw)
        buttons = []
        if Config.FORCE_CHANNEL:
