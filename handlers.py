...
# ========== UPDATED MEDIA MAPPING (YOUR IDs) ==========
# NOTE: Agar koi image kaam nahi karti, toh "/getfileid" se naya ID lein aur yahan update karein.
MEDIA_MAPPING = {
...
class BotHandlers:
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=False):
        ...
        # If image_id is None, we send text only
        if not image_id:
            logger.warning(f"No image ID for screen: {screen}")
            if is_edit:
                await msg.edit_text(
                    text=caption + f"\n\n⚠️ No image configured for this screen.",
