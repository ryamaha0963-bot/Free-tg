...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def callback_handler(client, callback):
            ...
            # ---------- SCREEN NAVIGATION ----------
            # Map callback data to screen names
            screen_map = {
                "welcome": "welcome",
                "profile": "profile",
                "invite": "invite",
    ...
    async def _show_screen(self, client, target, screen, user_id):
        """Delete old message and send new one with correct image + caption for the given screen."""
        # Delete old message if it exists and is not the original command message
        try:
        ...
        # ----- SEND NEW MESSAGE WITH PHOTO (or fallback to text) -----
        ...
        try:
            ...
            logger.error(f"Photo send failed for screen {screen}: {e}")
            await client.send_message(
                chat_id=chat_id,
                text=caption + "\n\n⚠️ Image could not be loaded. (Screen: " + screen + ")",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode=ParseMode.MARKDOWN
            )
