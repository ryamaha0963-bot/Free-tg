...
# ---------- TEMPORARY FILE IDs (Placeholder – Update after getting real ones) ----------
MEDIA_MAPPING = {
    "welcome": "",
    "profile": "",
    "invite": "",
...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def start_cmd(client, message):
            ...
            # Delete previous message if any and send fresh
            try:
                await message.delete()
            except:
                pass
            await self._show_screen(client, message, "welcome", user_id, is_edit=False, reply_to=message)
        ...
        async def callback_handler(client, callback):
            ...
            if data == "auto_switch":
                ...
                # Show welcome screen again (it will update with new button label)
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=True, reply_to=None):
        """Show any screen – if edit fails, delete old and send new."""
        diamonds = await get_diamonds(user_id)
        ...
        # --- Try to edit; if fails, delete old and send new ---
        try:
            if is_edit and target:
                try:
                    ...
                    logger.error(f"Edit failed for screen {screen}: {e}")
            ...
            logger.error(f"Fatal error showing screen {screen}: {e}")
