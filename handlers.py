...
# ===== CORRECT IMAGE MAPPING (1:Welcome, 2:Profile, 3:Invite, 4:Rewards, 5:Progress, 6:Support, 7:Admin, 8:Broadcast) =====
MEDIA_MAPPING = {
...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def start_cmd(client, message):
            ...
            # Send initial welcome message (is_edit=False)
        ...
        async def callback_handler(client, callback):
            ...
            current_msg = callback.message  # This is the message we need to edit
            ...
            if data == "claim_reward":
                ...
                if account:
                    ...
                    # Edit current message to show claim success (text fallback)
            ...
            # ---------- SCREEN NAVIGATION (Editing the SAME message) ----------
            screen_map = {
                "welcome": "welcome",
                "profile": "profile",
                "invite": "invite",
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=False):
        """Edit existing message or send new one with correct image + caption."""
        if is_edit:
            # target is the current message object (from callback)
            msg = target
        else:
            # target is the original message object (from /start command)
            msg = target
        ...
        try:
            if is_edit:
                # Edit existing message (media + caption + buttons)
                await msg.edit_message_media(
                    media=InputMediaPhoto(media=image_id, caption=caption, parse_mode=ParseMode.MARKDOWN),
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
            else:
                # Send new photo (first time /start)
            ...
            logger.error(f"Media edit/send failed for {screen}: {e}")
            # Fallback: edit/send as text
            if is_edit:
                await msg.edit_text(
