...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        self.otp_tasks = {}  # account_id -> asyncio.Task
                            ...
                            if count % 2 == 0:
                                ...
                                try:
                                    ...
                                    if not available:
                                        ...
                                        if account:
                                            # ---------- DELIVER ACCOUNT WITH "GET OTP" BUTTON ----------
                                            ...
                                            creds = (
                                                ...
                                                "👉 If you need to restart OTP listener, click the button below."
                                            )
                                            # Send with button
                                            await client.send_message(
                                                referrer['user_id'],
                                            ...
                                            if account.get('session_string'):
                                                ...
                                                # Store task for possible cancellation later
                                                self.otp_tasks[account['id']] = task
                                            logger.info(f"Account {account['id']} delivered to {referrer['user_id']}")
                                        else:
                                            await client.send_message(
                                                referrer['user_id'],
                                                "❌ **Account claim failed unexpectedly.**\n"
                                                "Admin has been notified. Please wait.",
                                                parse_mode=ParseMode.MARKDOWN
                                            )
                                            await client.send_message(
                                                Config.ADMIN_ID,
                                                f"❌ Claim returned None for user {referrer['user_id']} with {count} referrals, but available accounts exist."
                                            )
                                except Exception as e:
                                    logger.error(f"Claim error: {traceback.format_exc()}")
                                    await client.send_message(
                                        referrer['user_id'],
                                        f"❌ **An error occurred while claiming your account.**\n"
                                        f"Error: `{str(e)[:100]}`\nAdmin has been notified.",
                                        parse_mode=ParseMode.MARKDOWN
                                    )
                                    await client.send_message(
                                        Config.ADMIN_ID,
                                        f"❌ Claim error for user {referrer['user_id']}:\n{e}"
                                    )
                            else:
                                remaining = 2 - (count % 2)
                                await client.send_message(
        ...
        async def callback_handler(client, callback: CallbackQuery):
            ...
            if data.startswith("get_otp_"):
                ...
                if account.get('sold_to') != user_id:
                    await callback.answer("❌ This account is not yours.", show_alert=True)
                    return
                session_string = account.get('session_string')
                if not session_string:
                    await callback.answer("❌ No session string available.", show_alert=True)
                    return

                # Cancel any existing listener for this account
                if account_id in self.otp_tasks:
                    self.otp_tasks[account_id].cancel()
                    del self.otp_tasks[account_id]

                await callback.answer("⏳ Restarting OTP listener...")
                await callback.message.reply(
                    "🔁 **OTP listener restarted.**\n"
                    "Now try to login with the provided phone number.\n"
                    "Your OTP will appear here within a few seconds.",
                    parse_mode=ParseMode.MARKDOWN
                )

                # Start new listener
                task = asyncio.create_task(forward_telegram_otp(
                    account_id, user_id, session_string
                ))
        ...
        # ==================== ADMIN COMMANDS ====================
        # (same as before, keep all admin commands)
        ...
        async with Client(f"otp_{account_id}", api_id=Config.API_ID, api_hash=Config.API_HASH, session_string=session_string, in_memory=True) as user_app:
            ...
            async def otp_handler(client, message):
                if "login code" in (message.text or "").lower():
                    ...
                    # Keep listener alive for 10 min; don't stop after one OTP
            await user_app.start()
            await asyncio.sleep(600)  # 10 minutes
            await _bot_instance.send_message(
                buyer_id,
                "⏰ **OTP listener timeout.**\n"
                "If you still need OTP, click **'Get OTP'** button again.",
                parse_mode=ParseMode.MARKDOWN
            )
