...
# ========== IMAGE MAPPING (STATIC IMAGES) ==========
MEDIA_MAPPING = {
    "welcome": "AgACAgUAAxkBAAICPWpiIWYRTlYmoNtf5YAMo4moOmHrAAIYE2sbpi4QV3bwmESz182zAAgBAAMCAAN5AAceBA",
    "profile": "AgACAgUAAxkBAAICQGpiIin66ZiZAAE8w5w-TfLrYKGNFwACGhNrG6YuEFcAARIpUDBtUDkACAEAAwIAA3kABx4E",  # Background image for profile
...
# ========== HELPER: OVERLAY TEXT ON IMAGE ==========
async def overlay_profile_info(client, user_id, base_file_id):
    """Download base image, overlay user info, return BytesIO"""
    try:
        ...
        # Box for text (dark semi-transparent)
        box_width = img.width - 100
        box_height = 300
        box_x = 50
        box_y = img.height - box_height - 60
        # Draw a rounded rectangle (simulate by drawing a rectangle with blur)
        draw.rectangle(
        ...
        # Draw text
        y = box_y + 20
        draw.text((box_x + 30, y), f"👤 {username}", font=font_title, fill=(255, 255, 255))
        y += 50
        draw.text((box_x + 30, y), f"💎 Diamonds: {diamonds}", font=font_big, fill=(255, 215, 0))
        y += 40
        draw.text((box_x + 30, y), f"👥 Referrals: {referrals}", font=font_medium, fill=(200, 200, 255))
        y += 35
        draw.text((box_x + 30, y), f"📱 Accounts Earned: {earned_count}", font=font_medium, fill=(200, 255, 200))
        y += 35
        draw.text((box_x + 30, y), f"🎯 Next Target: {next_target} diamonds", font=font_medium, fill=(255, 200, 200))
        ...
        logger.error(f"Profile overlay failed: {e}")
        return None
...
class BotHandlers:
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=False):
        ...
        captions = {
            ...
            "profile": "👤✨ **Your Dynamic Profile**",  # caption will be small, since image has data
            "invite": (
                "🔗✨ **Your Invite Link** ✨🔗\n\n"
        ...
        if screen == "profile" and image_id:
            try:
                ...
                if img_bytes:
                    ...
                    return
                # If overlay fails, fallback to normal static image
            except Exception as e:
                logger.error(f"Overlay failed, falling back to static: {e}")

        # ===== NORMAL STATIC IMAGE SEND =====
        try:
            if image_id:
                await client.send_photo(
                    chat_id=chat_id,
