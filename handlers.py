...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        # ---- Admin-only version of /available (redundant but kept for consistency) ----
        @app.on_message(filters.command("avail") & filters.user(Config.ADMIN_ID))
        async def avail_cmd(client, message):
            available = await get_available_accounts()
            count = len(available)
            await message.reply(f"📦 **Available Accounts:** `{count}`", parse_mode=ParseMode.MARKDOWN)
