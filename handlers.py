...
# ---------- CORRECT IMAGE FILE IDs (8 different images) ----------
MEDIA_MAPPING = {
...
class BotHandlers:
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=True):
        """Show any screen with its specific image."""
        diamonds = await get_diamonds(user_id)
        ...
        image_id = MEDIA_MAPPING.get(screen)  # ---> Yahan se specific image aayegi
        ...
        try:
            ...
            logger.error(f"Error showing screen {screen}: {e}")
            await target.reply(
