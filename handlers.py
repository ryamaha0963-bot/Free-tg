...
class BotHandlers:
    def __init__(self, app: Client):
        ...
        async def start_cmd(client, message):
            ...
            # Send initial welcome screen (new message, no deletion needed)
            await self._show_screen(client, message, "welcome", user_id, is_edit=False, old_message=None)
        ...
        async def callback_handler(client, callback):
            ...
            if data == "claim_reward":
                ...
                if account:
                    ...
                    # Send a new message with claim success, delete old
            ...
            if data == "auto_switch":
                ...
                await self._show_screen(client, old_msg, "welcome", user_id, is_edit=True, old_message=old_msg)
                return
            ...
            # For navigation screens: delete old and send new
            if data in ["welcome", "profile", "invite", "rewards", "progress", "support", "admin", "admin_broadcast", "my_rewards"]:
                await self._show_screen(client, old_msg, data, user_id, is_edit=True, old_message=old_msg)
                await callback.answer()
                return

            if data in ["admin_users", "admin_stats", "admin_settings"]:
                # Delete old and send under development
                await old_msg.delete()
                await callback.message.reply(
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit, old_message=None):
        """Show screen – deletes old message (if any) and sends a new one."""
        # Delete old message if editing
        if is_edit and old_message:
            try:
        ...
        # ---- Send new message with photo or text ----
        try:
            # If there is no old message (first time), target is the message object, else we send to same chat
            chat_id = target.chat.id if hasattr(target, 'chat') else target.chat.id
            # Actually target is either a Message or the original message object. For new send, we use target.reply_photo if it's a message.
            # But if we deleted old, we just send to the chat.
            ...
            logger.error(f"Failed to send photo for screen {screen}: {e}")
            # Fallback to text
            await client.send_message(
                chat_id=target.chat.id,
