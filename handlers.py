...
# ---------- IMAGE FILE IDs (Directly from your screenshot) ----------
MEDIA_MAPPING = {
    "welcome": "AgACAgUAAxxkBAAMSzWf_1_WELCOME_ASCEND",
    "profile": "AgACAgUAAxxkBAAMTzWf_2_PROFILE_ASCEND",
    "invite": "AgACAgUAAxxkBAAMUzWf_3_INVITE_ASCEND",
    "rewards": "AgACAgUAAxxkBAAMVzWf_4_REWARDS_ASCEND",
    "my_rewards": "AgACAgUAAxxkBAAMVzWf_4_REWARDS_ASCEND",
    "progress": "AgACAgUAAxxkBAAMWzWf_5_PROGRESS_ASCEND",
    "support": "AgACAgUAAxxkBAAMXzWf_6_SUPPORT_ASCEND",
    "admin": "AgACAgUAAxxkBAAMYzWf_7_ADMIN_ASCEND",
    "admin_broadcast": "AgACAgUAAxxkBAAMZzWf_8_BROADCAST_ASCEND",
}
...
class BotHandlers:
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=True):
        """Show any screen with image + caption + buttons"""
        ...
        image_id = MEDIA_MAPPING.get(screen, MEDIA_MAPPING["welcome"])

        if is_edit:
            try:
                await target.edit_message_media(
                    media=InputMediaPhoto(media=image_id, caption=captions.get(screen, ""), parse_mode=ParseMode.MARKDOWN),
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
            except Exception:
                await target.reply_photo(photo=image_id, caption=captions.get(screen, ""), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
        else:
            await target.reply_photo(photo=image_id, caption=captions.get(screen, ""), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN)
    ...
    async def admin_cmd(self, client, message):
        await message.reply(
            "🔧 **Admin Commands**\n\n"
    ...
    async def gen_session_cmd(self, client, message):
    ...
    async def complete_otp_cmd(self, client, message):
        parts = message.text.split()
        if len(parts) != 3:
    ...
    async def add_account_cmd(self, client, message):
    ...
    async def update_otp_cmd(self, client, message):
        parts = message.text.split()
        if len(parts) != 3:
    ...
    async def list_accounts_cmd(self, client, message):
    ...
    async def ref_stats_cmd(self, client, message):
        parts = message.text.split()
        if len(parts) != 2:
    ...
    async def broadcast_cmd(self, client, message):
