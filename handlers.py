...
# ---------- OTP FORWARDER USING TELETHON (NO "already connected" ERROR) ----------
async def forward_telegram_otp_telethon(account_id: int, buyer_id: int, session_string: str):
    ...
    await _bot_instance.send_message(
        buyer_id,
        "🔁 **OTP listener is active (Telethon).**\n"
        "Open Telegram app, enter the phone number, and press **'Next'**.\n"
        "The OTP will appear here automatically.\n\n"
    ...
    try:
        ...
        # Event handler for OTP messages
        ...
        try:
            ...
            logger.info(f"Telethon OTP listener cancelled for account {account_id}")
            raise

    except asyncio.CancelledError:
        raise
    except Exception as e:
        logger.error(f"Telethon OTP error for account {account_id}: {traceback.format_exc()}")
        await _bot_instance.send_message(
            buyer_id,
