...
# ---------- IMAGE FILE IDs (Update these after getting correct IDs via /getfileid) ----------
MEDIA_MAPPING = {
    "welcome": "AgACAgUAAxxkBAAMSzWf_1_WELCOME_ASCEND",   # Replace with correct ID
...
class BotHandlers:
    ...
    async def _show_screen(self, client, target, screen, user_id, is_edit=True):
        """Show any screen with image or fallback to text if image fails."""
        diamonds = await get_diamonds(user_id)
        ...
        try:
            if is_edit:
                # Editing existing message – try to edit media, if fails, edit caption only
                try:
                    ...
                    # If editing media fails, fallback to editing caption (send as text)
                ...
                # New message – send photo or text
                try:
            ...
            await target.reply(
                text=caption + "\n\n⚠️ Image could not be loaded. Please update file IDs.",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode=ParseMode.MARKDOWN
            )
